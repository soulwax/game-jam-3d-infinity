...
class PhysicalAftermathManager {
  ...
  PhysicalAftermathManager({NarrativeState? state})
  ...
  List<PhysicalAftermathItem> getActiveResidues() {
    ...
    if (state.hasFlag('ashworth.compact', 'accepted')) {
      items.add(const PhysicalAftermathItem(
        id: 'residue-broth',
        name: 'earthenware broth jug',
        locationRoom: 'hall',
        description: 'A stone jug with grease-proof paper tied with string around the rim.',
    ...
    if (state.hasFlag('denise.pears', 'taken')) {
      items.add(const PhysicalAftermathItem(
        ...
        description: 'Four small brown pears resting on a porcelain saucer on the sideboard.',
      ...
      items.add(const PhysicalAftermathItem(
        ...
        locationRoom: 'hall',
        description: 'A paper cone of garden pears left in the frost outside the draft excluder.',
    ...
    if (state.hasFlag('sylvia.certificate', 'granted')) {
      items.add(const PhysicalAftermathItem(
        ...
        locationRoom: 'living-room',
        description: 'A purple carbon copy of Quarantine Exemption Form 14-B.',
    ...
    if (state.hasFlag('residue.coal', 'cellar')) {
      items.add(const PhysicalAftermathItem(
        ...
        locationRoom: 'cellar',
        description: 'Rough hessian bags of Welsh anthracite slumped beside the coal chute.',
    ...
    if (state.hasFlag('telegram.08', 'read')) {
      items.add(const PhysicalAftermathItem(
        ...
        locationRoom: 'hall',
        description: 'A buff envelope with gummed paper strips across the fold.',
    ...
    if (state.hasFlag('truth.shawl', 'home')) {
      items.add(const PhysicalAftermathItem(
        ...
        description: 'A folded green-and-black wool shawl smelling faintly of cedar and coal smoke.',
    ...
    if (state.hasFlag('sowerby.paraffin', 'received')) {
      items.add(const PhysicalAftermathItem(
        ...
        description: 'A red tin container with a stamped brass spout on the scullery stone.',
    ...
    if (state.hasFlag('inspector.proclamation', 'acknowledged')) {
      items.add(const PhysicalAftermathItem(
        ...
        locationRoom: 'hall',
        description: 'An official Ministry notice tacked into the hall door frame.',
  ...
  List<PhysicalAftermathItem> getResiduesForRoom(String roomId) {
    return getActiveResidues().where((item) => item.locationRoom == roomId).toList();
  }
}
