...
class MaterialFamilyLibrary {
  ...
  void _add(MaterialDescriptor mat) {
    if (!mat.hasValidParameters) {
      throw StateError('Invalid material descriptor ${mat.id}');
    }
    if (_registry.containsKey(mat.id)) {
      throw StateError('Duplicate material descriptor ${mat.id}');
    }
  ...
  bool validateNeutralSceneLighting() {
    if (_registry.values.any((material) => !material.hasValidParameters)) {
      return false;
    }
    ...
    for (final mat in _registry.values) {
      ...
      if (!resp.r.isFinite || !resp.g.isFinite || !resp.b.isFinite) {
        return false;
      }
      // Do not mistake unique IDs for unique visible responses.
      final key =
          '${resp.r.toStringAsPrecision(12)},'
          '${resp.g.toStringAsPrecision(12)},'
          '${resp.b.toStringAsPrecision(12)}';
      if (!responseKeys.add(key)) return false;
    }
    return responseKeys.length == _registry.length;
  }
}
