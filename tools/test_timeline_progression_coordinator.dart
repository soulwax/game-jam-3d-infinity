...
void main() {
  ...
  final days = TimelineProgressionCoordinator.canonical21Days;
  check(days.length == 21, 'All 21 timeline days defined');
  check(days[0].day == 1 && days[0].title == 'Hospitality', 'Day 1 is Hospitality');
  ...
  final coordinator = TimelineProgressionCoordinator(state: state);

  final complianceEnding = coordinator.assembleEndingTexture('compliance');
  check(complianceEnding.endingType == 'compliance', 'Ending type is compliance');
  check(complianceEnding.measuredFacts.length == 2, 'Measured facts included');
  check(
    complianceEnding.relationalResidues.contains('Four pears placed on the sideboard.'),
    'Sideboard pear residue included in ending texture',
  );
  check(
    complianceEnding.relationalResidues.contains('Exemption slip signed for the mill boilers.'),
    'Signed exemption certificate residue included in ending texture',
  );

  // 3. Verify Synchronisation Ending Text
  final syncEnding = coordinator.assembleEndingTexture('synchronisation');
  check(
    syncEnding.closingTextureText.contains('Truth was distributed across flawed people'),
