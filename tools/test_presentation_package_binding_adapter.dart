...
void main() {
  ...
  check(
    adapter.count == 1 && received != null,
    'validated package reaches binding factory',
  );
  check(
    adapter.bindingFor(placement.id) == received,
    'binding is retained by placement ID',
  );
  adapter.detach(placement.id);
  check(adapter.count == 0, 'detach releases presentation binding');
  Object? rebound;
  adapter.attach(inventory(), placement, (resolution) {
    rebound = resolution.package.package;
    return rebound!;
  });
  check(
    adapter.count == 1 && rebound != null,
    'released placement can be rebound after context restoration',
  );
  adapter.dispose();
  check(adapter.count == 0, 'dispose releases all retained bindings');
